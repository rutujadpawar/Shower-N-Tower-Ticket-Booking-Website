...
def orders(invoice):
    if current_user.is_authenticated:
        ...
        for _key, product in orders.orders.items():
            ...
            grandTotal = ("%.2f" % (1.18 * float(subTotal)))
...
@app.route('/location')
def location():
    return render_template('products/location.html')
    return redirect(url_for('home'))
...
@app.route('/image')
def image():
    return render_template('products/image.html')
    return redirect(url_for('home'))
...
@app.route('/services')
def services():
    return render_template('products/services.html')
    return redirect(url_for('home'))
